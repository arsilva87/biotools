	...
	grid = NULL, latlong2km = TRUE, radius, nmin = NULL) 
{
    ...
    pb <- tcltk::tkProgressBar(title = "Spatial Gene Diversity", 
    ...
    if (marker.type == "codominant") {
       ...
       MaxDist <- NULL
       mHe <- matrix(nrow = length(id), ncol = length(o))
       ...
       for (i in 1:length(id)) {
          ...
          for(j in 1:length(o)) {
             if (n[i] == 0) { 
                ...
                mHe[i, j] <- fHeco(markers[id[[i]], o[j]:(o[j]+1)])
             }
          }
    ...
    diversity <- data.frame(coord.x = grid.[, 1], coord.y = grid.[, 2], 
       n, MaxDist, uHe, SE)
    out <- list(diversity = diversity, mHe = mHe, locations = loc.)
    class(out) <- "sHe"
    Sys.sleep(0.5)
    close(pb)
    return(out)
